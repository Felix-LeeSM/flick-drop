...
<Dialog.Root bind:open={open}>
	<Dialog.Portal>
		...
		/>
		<Dialog.Content
			class="fixed left-1/2 top-1/2 z-50 grid w-[18rem] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl border border-border bg-card p-5 pb-6 text-center text-card-foreground shadow-2xl outline-none data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95"
		>
			<div class="relative">
				<Dialog.Close
